...
class Server {
  ...
  #updateResponsePayload () {
    try {
      ...
      this.#responsePayload = template.replace('//SKETCH_SOURCE_URL', '/' + path.basename(this.#command.filePath))
  ...
  #watchFileForChanges () {
    ...
    fs.watch(this.#command.filePath, (event, fileName) => {
      if (fileName && event === 'change') {
        ...
        this.#sockets.forEach(socket => socket.write(this.#constructSocketMessage({ action: 'reload' })))
      }
    })
  }
